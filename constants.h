...
/*
 There also needs to be a pointer to each map because we need to be able to read in
 the name of a chord and determine which map to use.
 
 This can be done by having an array of pairs whose first value is a chord name and whose
 second value is a pointer to the correct map
 
 A map does "mess" up the order of the intervials (ex: M2 won't be right after m2), but that's
...
pair<string, map<string, string>> ARRAY_KEY[] = {
    {"c", C_FLAT}, {"b#", B_SHARP}, {"c#", C_SHARP}, {"db", D_FLAT}, {"d", D_NATURAL}, {"d#", D_SHARP},
